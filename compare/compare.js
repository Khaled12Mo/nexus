...
const games = [
  ...
  },
  {
    ...
    price: 59.99,
    multiplayer: "Yes",
    img: image("fifa-24.jpg"),
  },
  {
    id: 11,
    name: "Forza Horizon 5",
    genre: "Racing",
    platform: "Xbox / PC Exclusive",
  ...
  },
  {
    ...
    price: 14.99,
    multiplayer: "No",
    img: image("hollow-knight-silksong.jpg"),
  },
  {
    id: 21,
    name: "Mario Kart 8 Deluxe",
    genre: "Racing",
    platform: "Nintendo Exclusive",
    developer: "Nintendo",
    year: 2017,
    rating: 9.2,
    price: 59.99,
...
function selectcontent(gamesArray) {
  for (i = 0; i < gamesArray.length; i++) {
    for (j = 0; j < select.length; j++) {
      select[j].innerHTML +=
        `<option value="${gamesArray[i].name}">${gamesArray[i].name}</option>`;
    }
  }
}
selectcontent(games);
...
function filtergames() {
  if (search.value != "") {
    const filterd = games.filter((game) =>
    ...
    for (j = 0; j < select.length; j++) {
      if (select[j].value == "") {
        ...
        select[j].innerHTML =
          `<option value="${select[j].value}">${select[j].value}</option>`;
      }
    }
