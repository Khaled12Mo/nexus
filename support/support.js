  ...
  if (btn) {
    btn.addEventListener('click', () => {
      ...
      if (!callsign || !commlink || !message) {
        showToast('⚠️ Incomplete Data Packet — fill all required fields.', 'error');
        return;
      }
      ...
      setTimeout(() => {
        const ticketID = 'NX-' + Math.floor(Math.random() * 1000000).toString().padStart(6, '0');
        showToast('✅ Transmission successful — Ticket ' + ticketID + ' queued.', 'success');
